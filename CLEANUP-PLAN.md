...
> **Historical.** All items below are applied and committed, except §13 which was dropped.
> A `hostIsLive` fix was added mid-execution as §0 — see the note under Order.
>
> Kept because `BACKLOG.md` and `DECLINED.md` refer to its § numbers, and because the reasoning
> behind several deletions is not recoverable from the diffs. The **current** plan is
> [`PLAN.md`](./PLAN.md), designed in [`ARCHITECTURE.md`](./ARCHITECTURE.md).
