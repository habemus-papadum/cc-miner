...
| `app.asar` | 89.6 MB | `dist/` is 76 MB of it, almost all duckdb-wasm; the rest is the shell and electron-updater's tree |
| `app.asar.unpacked` | 112 MB | `libduckdb.dylib` — the native engine |
| **dmg / zip** | **~155 MB** | what a user downloads |

These are **constant** now, whatever the size of your history. `app.asar` used to
be 108–128 MB because the corpus was compiled into it; the artifact grew with
the data, and the 140 MB budget in `pack.mjs` was ~11 MB from firing on a
moderately large one. Moving the corpus out took that variable away entirely.
...
(A third lever, **~39 MB**, was taken outright: the corpus is no longer bundled at all. See below.)

## Two data modes over one corpus, declared not discovered

Both modes read the same corpus from **`~/.cache/cc-miner`**, generated by
`pnpm -C ../cc-assay mine`. They differ only in *where the query runs*, which is
what makes comparing their answers meaningful:
...
### The corpus is never bundled — enforced, not remembered

`/__corpus` is served by whichever host is running the page: the Vite dev server,
`pnpm preview`, or the packaged app's `app://` handler — one implementation, in
`server/host-runtime.mjs`, reading `~/.cache/cc-miner`.

It replaced an eager `import.meta.glob("../data/**/*.parquet")`. That glob was
resolved at **build** time, so `vite build` emitted every Parquet file as a
hashed asset and the corpus was compiled into the application. Measured: 121
session replay files in `dist/assets`, `app.asar` 128.5 MB against 89.6 MB
without — a `.dmg` handed to anyone carried the builder's own transcripts,
conversation bodies included. `src/data` being gitignored protected the
repository; nothing protected the artifact. The build now never sees the data,
so it cannot ship it.

`corpusFile()` resolves request paths and refuses anything escaping the corpus
root, plus the `.staging` intermediate — this route reads out of a home
directory, so its containment is pinned by `server/corpus-route.test.ts`.
...
| | |
| --- | --- |
| `~/.cache/cc-miner` | the default, on every platform — the same directory local mode reads |
| `$XDG_CACHE_HOME/cc-miner` | when that variable is set |
...
One spelling on every platform, deliberately — not `~/Library/Caches` on macOS.
This is a path that has to appear in shell commands, READMEs and error messages,
and being typeable everywhere is worth more than native placement for a directory
users are expected to point tools at.
